...
class Solution {
  public:
    vector<int> duplicates(vector<int> arr) {
        ...
        // Step 1: Create a map to count occurrences of elements
        unordered_map<int, int> counts;

        // Step 2: Count each element's occurrence
        for (int val : arr) {
            counts[val]++;
        }

        // Step 3: Store elements that occur more than once
        vector<int> result;
        ...
        // Step 4: Sort the result array
        sort(result.begin(), result.end());

        // Step 5: If no duplicates are found, return {-1}
        if (result.empty()) {
            return {-1};
        }
